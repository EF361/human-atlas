...
import {SYSTEMS,getPartColor,type Atlas,type SceneState} from './anatomy';
...
export default function AnatomyScene({atlas,state,onSelect,onProgress,onError}:Props){
 ...
 useEffect(()=>{
  ...
  atlas.parts.forEach((p,i)=>{const c=new T.Color(getPartColor(p));colorData[i*4]=c.r;colorData[i*4+1]=c.g;colorData[i*4+2]=c.b;colorData[i*4+3]=1;});
  const colorTexture=new T.DataTexture(colorData,width,1,T.RGBAFormat,T.FloatType);colorTexture.needsUpdate=true;
  ...
  const materialFor=(system:string)=>{
   const m=new T.MeshStandardMaterial({color:0xffffff,metalness:.08,roughness:.53,side:T.DoubleSide,transparent:system==='integumentary',opacity:system==='integumentary'?.1:1,depthWrite:system!=='integumentary'});
   m.onBeforeCompile=shader=>{
    shader.uniforms.partState={value:partTexture};shader.uniforms.selectionState={value:selectionTexture};shader.uniforms.colorState={value:colorTexture};shader.uniforms.stateWidth={value:width};
    shader.vertexShader='attribute float partIndex; uniform sampler2D partState; uniform sampler2D selectionState; uniform sampler2D colorState; uniform float stateWidth; varying float partVisible; varying float partSelected; varying vec3 vPartColor;\n'+shader.vertexShader;
    shader.vertexShader=shader.vertexShader.replace('#include <begin_vertex>','#include <begin_vertex>\nvec2 stateUv = vec2((partIndex + 0.5) / stateWidth, 0.5); vec4 state = texture2D(partState, stateUv); transformed += state.xyz; partVisible = state.w; partSelected = texture2D(selectionState, stateUv).r; vPartColor = texture2D(colorState, stateUv).rgb;');
    shader.fragmentShader='varying float partVisible; varying float partSelected; varying vec3 vPartColor;\n'+shader.fragmentShader;
    shader.fragmentShader=shader.fragmentShader.replace('#include <clipping_planes_fragment>','#include <clipping_planes_fragment>\nif (partVisible < 0.5) discard;');
    shader.fragmentShader=shader.fragmentShader.replace('#include <color_fragment>','#include <color_fragment>\ndiffuseColor.rgb = vPartColor;\ndiffuseColor.rgb = mix(diffuseColor.rgb, diffuseColor.rgb * 1.35 + vec3(0.08, 0.12, 0.12), partSelected * 0.45);');
  ...
  const clock=new T.Clock();let lastExtent=-1,lastCrossSection=-1,lastCrossSectionAxis='';
  const animate=()=>{
   ...
   const cs=s.crossSection??0,csAxis=s.crossSectionAxis??'coronal';
   if(cs!==lastCrossSection||csAxis!==lastCrossSectionAxis||((isolateKey!==lastIsolate||changed)&&cs>0)){
    dirty=true;lastCrossSection=cs;lastCrossSectionAxis=csAxis;
    if(cs>0){
     const box=new T.Box3();
     if(s.isolate&&s.selected.length>0){atlas.parts.forEach((p,i)=>{if(s.selected.includes(p.id))box.union(bounds[i].clone().translate(new T.Vector3(data[i*4],data[i*4+1],data[i*4+2])));});}
     if(box.isEmpty()){box.set(new T.Vector3(-.3,0,-.2),new T.Vector3(.3,1.8,.2));}
     const center=box.getCenter(new T.Vector3()),size=box.getSize(new T.Vector3());
     if(csAxis==='sagittal'){clipPlane.normal.set(1,0,0);clipPlane.constant=-(center.x-size.x*.5+size.x*cs);}
     else if(csAxis==='axial'){clipPlane.normal.set(0,-1,0);clipPlane.constant=center.y-size.y*.5+size.y*(1-cs);}
     else{clipPlane.normal.set(0,0,-1);clipPlane.constant=center.z+size.z*.5-size.z*cs;}
     materials.forEach(m=>{if(!m.clippingPlanes||m.clippingPlanes.length===0){m.clippingPlanes=[clipPlane];m.needsUpdate=true;}});
    }else{materials.forEach(m=>{if(m.clippingPlanes&&m.clippingPlanes.length>0){m.clippingPlanes=[];m.needsUpdate=true;}});}
   }
